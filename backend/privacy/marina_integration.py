...
class AdaMarinaIntegration:
    ...
    def __init__(self, privacy_core: AdaSeaPrivacyCore):
        ...
        self.default_permissions = {"automatic": False, "requires_confirmation": True}  # Still requires confirmation

    async def request_berth_assignment(
        self, marina_id: str, vessel_specs: Dict[str, Any], arrival_time: str
    ) -> Dict[str, Any]:
        """
    ...
    async def inform_arrival(
        self, marina_id: str, vessel_name: str, current_position: Dict[str, float], berth_number: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    async def request_services(
        self, marina_id: str, services: List[str], vessel_info: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    async def check_compliance(self, marina_id: str, vessel_docs: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def emergency_notification(
        self, marina_id: str, emergency_type: str, vessel_info: Dict[str, Any], position: Dict[str, float]
    ) -> Dict[str, Any]:
        """
    ...
    def setup_standing_permission(
        self, marina_id: str, permission_type: str, expiry_hours: int = 168  # 7 days default
    ) -> Dict[str, Any]:
        """
        ...
        if permission_type not in permission_map:
            return {"success": False, "reason": f"Invalid permission type: {permission_type}"}

        # Create standing permission request
        # This would integrate with consent manager
        return {
            "success": True,
            "message": (f"✓ {marina_id} için {permission_type} " f"otomatik paylaşım aktif ({expiry_hours} saat)"),
            "destination": f"Ada.marina:{marina_id}",
    ...
    def get_marina_permissions(self) -> List[Dict[str, Any]]:
        ...
        marina_permissions = [p for p in all_permissions if p["destination"].startswith("Ada.marina:")]
    ...
    def revoke_marina_permission(self, marina_id: str) -> int:
        ...
        for perm in permissions:
            if marina_id in perm["destination"]:
                self.privacy_core.revoke_standing_permission(perm["destination"], perm["data_type"])
                count += 1

        return count

    def get_marina_sharing_history(self, marina_id: Optional[str] = None, hours: int = 168) -> List[Dict[str, Any]]:
        """
        ...
        return self.privacy_core.get_audit_trail(destination=destination_filter, hours=hours)


# Voice command handlers for marina integration
MARINA_VOICE_COMMANDS = {
    "tr": {
        "check_in": ["marina'ya check-in yap", "marina'ya bildir", "varışımı bildir"],
        "berth_request": ["berth reserve et", "yer ayırt", "yer rezervasyonu"],
        "service_request": ["servis talep et", "yakıt iste", "bakım iste"],
        "show_permissions": ["marina izinlerini göster", "marina paylaşımlarını göster"],
        "revoke_all": ["marina izinlerini iptal et", "otomatik paylaşımı durdur"],
    },
    "en": {
        "check_in": ["check in to marina", "notify marina", "inform arrival"],
        "berth_request": ["reserve berth", "request berth", "book berth"],
        "service_request": ["request service", "request fuel", "request maintenance"],
        "show_permissions": ["show marina permissions", "show marina sharing"],
        "revoke_all": ["revoke marina permissions", "stop automatic sharing"],
    },
}
